package clock.view.size.layout;
...
public class VerticalLayout extends VBox implements ISizeManagedLayout {
	...
	public VerticalLayout(final ISizeManagedNode... sizeManagedNodes) {
		...
		minimumSizeManager = new MinimumSizeManager(new Function<Double, Dimension2D>() {
			...
			public Dimension2D apply(Double fontSize) {
				...
				for (ISizeManagedNode sizeManagedNode : sizeManagedNodes) {
					...
					minimumWidth = Math.max(minimumWidth, minimumSize.getWidth());
	...
	public void setSize(double newWidth, double newHeight, double newFontSize) {
		...
		for (ISizeManagedNode fontSizeBasedNode : sizeManagedNodes) {
			...
			sumWidth = Math.max(sumWidth, minimumSize.getWidth());
			sumHeight += minimumSize.getHeight();
		}
		...
		for (ISizeManagedNode fontSizeBasedNode : sizeManagedNodes) {
			...
			fontSizeBasedNode.setSize(sumWidth, size.getHeight(), newFontSize);
		}
	}
