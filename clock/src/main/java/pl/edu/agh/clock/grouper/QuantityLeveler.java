...
public final class QuantityLeveler {
	private static final Logger log = Logger.getLogger(QuantityLeveler.class);

	private QuantityLeveler() {
		throw new AssertionError();
	}

	public static QuantityLevelProvider getQuantityLevelProvider(double minimumLevelValue, double maximumLevelValue, int minimumInputValue,
	...
	public static final class QuantityLevelProvider {
		...
		private QuantityLevelProvider(double minimumLevelValue, double maximumLevelValue, int minimumInputValue, int maximumInputValue) {
			...
			double levelValueDelta = (maximumLevelValue - minimumLevelValue) / (maximumGranulationLevel - minimumGranulationLevel);
			...
			for (int currentValue = minimumGranulationLevel; currentValue <= maximumGranulationLevel; currentValue++) {
				granulationMap.put(currentValue, new QuantityLevel(currentLevelValue, getLevelDescription(currentValue * granulationUnit)));
				currentLevelValue += levelValueDelta;
			}

			log.debug("------------- Granulation unit selection -------------"); //$NON-NLS-1$
			log.debug("Min input value: " + minimumInputValue); //$NON-NLS-1$
			log.debug("Max input value: " + maximumInputValue); //$NON-NLS-1$
			log.debug("Granulation unit: " + granulationUnit); //$NON-NLS-1$
			log.debug("-------------------------------------------------------"); //$NON-NLS-1$
		}
		...
		private String getLevelDescription(int currentValue) {
			if (granulationUnit == 1) {
				...
				return Joiner.on(" - ").join(currentValue, currentValue + granulationUnit - 1); //$NON-NLS-1$
			}
		}
		...
		private int getGranulationUnit(int minimumInputValue, int maximumInputValue) {
			...
			for (int granulationLevelPretendant : getGranulationUnits()) {
				...
				double granulationLevelPretendantEfficiency = (double) numberOfGroupsForGivenGranulationLevel / TARGET_NUMBER_OF_GROUPS;
